 ...
 ll DP(ll cur,int lvl,int pColor)
{
    ...
    {
        ll child=(1ll << lvl)-2;
        return dp[pColor][cur]=(1ll * cnt * BigMod(4,child))%mod;
    }
    ll left=cur << 1ll;
    ll right=left+1,ans=0;
    if(colored[cur].find(cur) == colored[cur].end())
    {
        for(int i=1;i<7;i++)
        {
    ...
    else
    {
        ...
        if(Ok(clr,pColor)  &&  Ok(clr,colored[cur][left])  &&  Ok(clr,colored[cur][right]))
        {
            ans += DP(left,lvl-1,clr) * DP(right,lvl-1,clr);
            ans %= mod;
        }
    }
