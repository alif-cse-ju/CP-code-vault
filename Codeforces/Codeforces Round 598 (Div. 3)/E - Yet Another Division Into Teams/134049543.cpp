...
const int mod=1e9+7;
pair<int,int>a[M];
pair<ll,int>node[4*M];
void Build(int nodeNum,int l,int r)
    if(l == r)
    {
        node[nodeNum]={LLONG_MAX,INT_MAX};
        return;
    }
    int mid=(l+r) >> 1;
...
void Update(int nodeNum,int start,int end,int pos,ll v)
{
    if(start == end)
    {
        node[nodeNum]={v,pos};
 ...
 pair<ll,int>Query(int nodeNum,int start,int end,int l,int r)
{
