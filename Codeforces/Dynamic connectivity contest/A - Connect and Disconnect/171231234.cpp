 ...
 void Solve(int nodeNum, int l, int r)
{
    ...
    for(int it : segTree[nodeNum])
    {
        ...
        {
            --conComponent;
            if(sz[px] >= sz[py])sz[px]+=sz[py], par[py]=px, updates.emplace_back(make_tuple(px,py));
            else sz[py]+=sz[px], par[px]=py, updates.emplace_back(make_tuple(py,px));
        }
    }
    ...
    {
        ++conComponent;
        auto [px,py]=updates.back(); updates.pop_back();
        par[py]=py, sz[px]-=sz[py];
    }
}
 int main()
{
