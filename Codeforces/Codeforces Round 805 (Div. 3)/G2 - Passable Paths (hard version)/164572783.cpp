...
int main()
{
    ...
    for(int qc=1;qc<=q;qc++)
    {
            ...
            for(int &y : v)
            {
                    if(inTime[y] <= inTime[x1]  &&  outTime[y] >= outTime[x1])mark[y]=1, --k;
            }
            if(!k)cout << "YES\n";
            else
            {
                    ...
                    for(int &y : v)
                    {
                            ...
                            {
                                    if(!x2  ||  inTime[x2] < inTime[y])x2=y;
                            }
                    }
                    bool ok=1;
                    for(int &y : v)
                    {
                            if(mark.find(y) == mark.end())ok = ok & ((inTime[y] <= inTime[x2])  &  (outTime[y] >= outTime[x2]));
                    }
                    int lca=LCA(x1,x2);
                    if(ok)
                    {
